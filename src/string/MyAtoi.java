...
//        System.out.println(myAtoi.myAtoi("2147483648"));
    ...
    public int myAtoi(String str) {
        ...
        char char0 = str.charAt(start);
        if ((char0 > '9' || char0 < '0') && char0 !='-' && char0 != '+') {
            return 0;
        }
        boolean negative = char0 == '-';
        int length = start;
        if (negative || char0 == '+') {
            length++;
        }
        ...
        for(;length <str.length(); length++) {
            ...
            result = result * 10;
            if (multiplyExact(result)) {
                return negative ? Integer.MIN_VALUE : Integer.MAX_VALUE;
            }
            int old = result;
            result = result + (c - '0');
            if (addExact(old, (c - '0'), result)) {
                return negative ? Integer.MIN_VALUE : Integer.MAX_VALUE;
            }
            if (result < old) {
                return negative ? Integer.MIN_VALUE : Integer.MAX_VALUE;
    ...
    public boolean addExact(int x, int y, int result) {
        // HD 2-12 Overflow iff both arguments have the opposite sign of the result
        return ((x ^ result) & (y ^ result)) < 0;
    }

    public boolean multiplyExact(int x) {
        return x % 10 != 0;
    }
